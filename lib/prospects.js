...
export function rowToProspect(row) {
  return {
    ...
    contractDetails:
      row.contract_details && typeof row.contract_details === 'object'
...
export function prospectToRow(p) {
  const row = {
    ...
    contract_details:
      p.contractDetails && typeof p.contractDetails === 'object'
        ? p.contractDetails
        : {},
    opening_hours:
...
export async function insertProspect(prospect) {
  ...
  if (prospect.addedBy) row.added_by = prospect.addedBy
  const { data, error } = await supabase
    .from('prospects')
    .insert(row)
    .select()
...
/** Abonnement Realtime — émet 'prospects-updated' sur window */
export function subscribeProspectsRealtime() {
    ...
    .on(
      ...
      function (payload) {
        window.dispatchEvent(
          new CustomEvent('prospects-updated', { detail: payload })
        )
      }
    )
    .subscribe()
